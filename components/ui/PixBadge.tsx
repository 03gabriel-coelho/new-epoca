...
const PixIcon: React.FC<{ className?: string }> = ({
...
const PixBadge: React.FC<PixBadgeProps> = ({
  label = "no PIX",
  className = "",
}) => {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-emerald-700 ${className}`}
    >
      <PixIcon />
      {label}
    </span>
